classdef DDQN_block < matlab.System & matlab.system.mixin.Propagates ...
        &  matlab.system.mixin.Nondirect & ...
        matlab.system.mixin.SampleTime
    % Double DQN agent for falsification
...
%     end
    ...
    methods(Access = protected)
        ...
        function resetImpl(obj)
            ...
            coder.extrinsic('py.DDQN_agent.act')
            action_normalized = double(py.DDQN_agent.act([-1 -1 -1]));
        ...
        function updateImpl(obj, state, reward)
            coder.extrinsic('py.DDQN_agent.update')
            action_normalized = [0 0];
            t = getCurrentTime(obj);
            if floor(t ./ obj.sample_time) ~= floor(obj.last_t ./ obj.sample_time)
                action_normalized = double(py.DDQN_agent.update(state', reward(1)));
                action_normalized = min([1.0 1.0], max([-1.0 -1.0], action_normalized));
                lower = obj.input_range(:,1)';
                upper = obj.input_range(:,2)';
                middle = (lower + upper)/2.0;
